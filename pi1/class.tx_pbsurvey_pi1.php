...
class tx_pbsurvey_pi1 extends tslib_pibase {
    ...
    function markerList($arrQuestion,$strTemplate) {
        if (in_array($arrQuestion['question_type'],array(1,2,3))) {
            ...
            $arrVars = array($this->pi_getLL($arrLLVals[0]),$this->pi_getLL($arrLLVals[1]));
			foreach ($arrVars as $intKey => $strItem){
				unset($arrQuestion['checked']);
            	unset($arrQuestion['selected']);
				$arrQuestion['counter'] = $intKey+1;
				$arrQuestion['value'] = $strItem;
				if ($this->checkUpdate($arrQuestion['uid']) && $intKey+1==$this->arrUserData[$arrQuestion['uid']]) {
					$arrQuestion['checked'] = 'checked="checked"';
					$arrQuestion['selected'] = 'selected="selected"';
				} elseif ($intKey+1==$arrQuestion[$arrLLVals[2]]) {
					$arrQuestion['checked'] = 'checked="checked"';
					$arrQuestion['selected'] = 'selected="selected"';
				}
				$strOutput .= $this->cObj->substituteMarkerArray($GLOBALS['TSFE']->cObj->getSubpart($strTemplate, '###ENTRY###'), $arrQuestion, '###|###', 1);
